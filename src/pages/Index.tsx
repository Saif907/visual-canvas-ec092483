...
import {
  Plus, Trash2, GripVertical, X, ChevronDown,
...
const DEFAULT_DASH_ID = "default-dashboard";
...
// ──────────────────────────────────────────────
// Default overview dashboard (the old Index layout)
// ──────────────────────────────────────────────
function createOverviewDashboard(): Dashboard {
  return {
    id: DEFAULT_DASH_ID,
    name: "Overview",
    isDefault: true,
    widgets: [
      ...
      { widgetId: "account-balance", layout: { x: 6, y: 0, w: 3, h: 2 } },
      { widgetId: "trade-count", layout: { x: 9, y: 0, w: 3, h: 2 } },
      { widgetId: "profit-factor", layout: { x: 0, y: 2, w: 3, h: 2 } },
      { widgetId: "total-volume", layout: { x: 3, y: 2, w: 3, h: 2 } },
      { widgetId: "avg-holding-time", layout: { x: 6, y: 2, w: 3, h: 2 } },
      { widgetId: "win-loss-streak", layout: { x: 9, y: 2, w: 3, h: 2 } },
      { widgetId: "chart-balance-history", layout: { x: 0, y: 4, w: 6, h: 3 } },
      { widgetId: "chart-daily-pnl", layout: { x: 6, y: 4, w: 6, h: 3 } },
    ],
    createdAt: new Date().toISOString(),
  };
}

function createBlankDashboard(name: string): Dashboard {
  return {
...
const Index = () => {
  ...
  const [dashboards, setDashboards] = useState<Dashboard[]>(() => {
    ...
    const hasDefault = loaded.some(d => d.id === DEFAULT_DASH_ID);
    if (loaded.length === 0) {
      const overview = createOverviewDashboard();
      saveDashboards([overview]);
      setActiveDashboardId(overview.id);
      return [overview];
    }
    if (!hasDefault) {
      const overview = createOverviewDashboard();
      const next = [overview, ...loaded];
      saveDashboards(next);
      return next;
    }
    return loaded;
  ...
  const handleCreateDashboard = useCallback(() => {
    const newDash = createBlankDashboard(`Dashboard ${dashboards.length}`);
  ...
  const handleResetDashboard = useCallback(() => {
    ...
    const overview = createOverviewDashboard();
    setDashboards(prev => prev.map(d =>
      d.id === activeDashboard.id ? { ...d, widgets: overview.widgets } : d
    ));
  }, [activeDashboard]);
      ...
      <main className={cn("transition-all duration-300 p-6 pb-16", sidebarCollapsed ? "ml-[80px]" : "ml-[280px]")}>
                    ...
                    <div className="p-1 max-h-48 overflow-y-auto">
                      {dashboards.map((d) => (
                        ...
                        >
                          ...
                          {d.isDefault && <span className="text-[10px] text-text-secondary bg-muted px-1.5 py-0.5 rounded">Default</span>}
        ...
        {activeDashboard && activeDashboard.widgets.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center min-h-[60vh]">
            ...
            >
              Or start with the default layout
            </button>
          </div>
        )}
      </main>
